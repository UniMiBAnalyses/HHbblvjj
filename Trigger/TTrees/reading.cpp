...
int main(int argc, char** argv){
	...
	double jmax_pt, jmax_eta;

	//Energy released and missing enegy
	double HT, MET, MHT, MHTNoMu, METNoMu;
	...
	//Maximum values from the four jets
	tree->Branch("jmax_pt", &jmax_pt); 
	tree->Branch("jmax_eta", &jmax_eta);

	//Energy released and missing enegy	
	tree->Branch("HT", &HT);
	tree->Branch("MET", &MET);
	tree->Branch("MHT", &MHT);
	tree->Branch("MHTNoMu", &MHTNoMu);
	tree->Branch("METNoMu", &METNoMu);
	...
	while ( reader.readEvent() ){
		...
		for (int iPart = 0 ; iPart < reader.hepeup.IDUP.size (); iPart++){
			...
			if (reader.hepeup.ISTUP.at (iPart) == 1){
				...
				if ( abs(ID) < 7 && abs(ID) != 5 ){ 
					...
					/*
					| Particle index is inserted in the vector: the size of the vector will say 
					| how many jets are produced and elements in the vector can be used to find
					| the TLorentzVector in the map 
					*/ 	
		...
		if(jets.size() == 2 && tau == 0){      

			j1_mom = momenta[jets[0]];
			j2_mom = momenta[jets[1]];
			...
			vector<double> jpt {j1_pt, j2_pt, jb1_pt, jb2_pt};

			auto maximum = max_element(begin(jpt), end(jpt));
			jmax_pt =*maximum;
			...
			//Saving MHT
			vector<TLorentzVector> moms {jb1_mom, jb2_mom, j1_mom, j2_mom, mu_mom, ele_mom};

			TLorentzVector moms_sum (0., 0., 0., 0.); 

			for (int iMom = 0; iMom < moms.size(); ++iMom) {
			...
			//Saving MET and MHT (this time muons are not considered)
			vector<TLorentzVector> moms_NoMu {jb1_mom, jb2_mom, j1_mom, j2_mom, ele_mom};

			TLorentzVector moms_sum_NoMu (0., 0., 0., 0.); 

			for (int jMom = 0; jMom < moms_NoMu.size(); ++jMom) {

				if ( moms_NoMu[jMom].Pt() > 20 ) {

					moms_sum_NoMu += moms_NoMu[jMom];
				}
			}

			MHTNoMu = moms_sum_NoMu.Pt();
			
			METNoMu = (jb1_mom + jb2_mom + j1_mom + j2_mom + ele_mom).Pt();
