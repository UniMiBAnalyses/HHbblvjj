...
int main(int argc, char** argv){
	...
	double et_ele_n_mass, mt_mu_n_mass;
	...
	//Maximum values from the four jets
	double mt_jmax_pt, mt_jmax_eta;
	double et_jmax_pt, et_jmax_eta;

	//Energy released and missing enegy
	double mt_HT, mt_MET, mt_MHT;
	double et_HT, et_MET, et_MHT;
	
	double mt_MHTNoMu, mt_METNoMu;
	double et_MHTNoMu, et_METNoMu;
	...
	ele_tree->Branch("ele_n_mass", &et_ele_n_mass);
	mu_tree->Branch("mu_n_mass", &mt_mu_n_mass);	
	...
	//Maximum values from the four jets
	mu_tree->Branch("jmax_pt", &mt_jmax_pt);
	mu_tree->Branch("jmax_eta", &mt_jmax_eta);
	
	ele_tree->Branch("jmax_pt", &et_jmax_pt); 
	ele_tree->Branch("jmax_eta", &et_jmax_eta);
	...
	mu_tree->Branch("MHTNoMu", &mt_MHTNoMu);
	mu_tree->Branch("METNoMu", &mt_METNoMu);

	ele_tree->Branch("MHTNoMu", &et_MHTNoMu);
	ele_tree->Branch("METNoMu", &et_METNoMu);

	mu_tree->Branch("MHT", &mt_MHT);
	ele_tree->Branch("MHT", &et_MHT);
	...
	TTreeReaderValue<double>  ele_n_mass_value( reader, "ele_n_mass" );
	TTreeReaderValue<double>  mu_n_mass_value( reader, "mu_n_mass" );
	...
	//Maximum values from the four jets
	TTreeReaderValue<double> jmax_pt_value( reader, "jmax_pt" );
	TTreeReaderValue<double> jmax_eta_value( reader, "jmax_eta" );

	//Energy released and missing enegy
	TTreeReaderValue<double>  HT_value( reader, "HT" );
	TTreeReaderValue<double>  MHT_value( reader, "MHT" );
	TTreeReaderValue<double>  MET_value( reader, "MET" );
	
	TTreeReaderValue<double>  MHTNoMu_value( reader, "MHTNoMu" );
	TTreeReaderValue<double>  METNoMu_value( reader, "METNoMu" );
	...
	while (reader.Next()) {
		...
		if (*mu_pt_value == 0) {
				...
				et_ele_n_mass = *ele_n_mass_value; 
				...
				//Maximum values from the jets
				et_jmax_pt = *jmax_pt_value;
				et_jmax_eta = *jmax_eta_value;
				...
				et_MHTNoMu = *MHTNoMu_value;
				et_METNoMu = *METNoMu_value;
		...
		if (*ele_pt_value == 0) {
				...
				//Maximum values from the jets
				mt_jmax_pt = *jmax_pt_value;
				mt_jmax_eta = *jmax_eta_value;
				...
				mt_MHTNoMu = *MHTNoMu_value;
				mt_METNoMu = *METNoMu_value;
