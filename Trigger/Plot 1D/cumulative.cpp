...
int main(int argc, char** argv){
	...
	if (argc < 2){
		cout << "Usage: " << argv[0] << " variable " << endl;
		return 1;
	}
	...
	cout << "E' stato prodotto un muone o un elettrone? " << endl;
	...
	/*
	| Calcolo anche l'efficienza totale ( cioè l'efficienza calcolata considerando il numero totale di eventi).
	| Per i trigger considerati è sufficiente limitarsi al caso "x > threshold"
	*/

	double tot_efficiency = ( h2_efficiency -> GetBinContent(threshold) ) * ( hs->GetEntries() ) / 1000000;
	...
	if (var[0] == "mu_n_mass" || var[0] == "ele_n_mass") { 
